...
print("Reading data...")
print(f"Sampling per class: {samples_per_class} Total samples: {samples_per_class * 10}")
df = pd.read_csv(os.path.join(path, 'UNSW-NB15.csv'))
...
new_data_pca['Label'] = attacks.values  # Using Label instead of Attack Type
...
new_data_k_32['Label'] = attacks.values  # Using Label instead of Attack Type

print("Applying SMOTE to balance data...")
def balance_and_split(dataset):
    class_counts = dataset['Label'].value_counts()  # Changed to Label
    selected_classes = class_counts[class_counts > 2000]
    class_names = selected_classes.index
    selected = dataset[dataset['Label'].isin(class_names)]  # Changed to Label
    
    dfs = []
    for name in class_names:
        df_class = selected[selected['Label'] == name]  # Changed to Label
    ...
    X = balanced_df.drop('Label', axis=1)  # Changed to Label
    y = balanced_df['Label']  # Changed to Label
    smote = SMOTE(sampling_strategy='auto', random_state=0)
    X_upsampled, y_upsampled = smote.fit_resample(X, y)
    balanced_data = pd.DataFrame(X_upsampled)
    balanced_data['Label'] = y_upsampled  # Changed to Label
    balanced_data = balanced_data.sample(frac=1)
    return train_test_split(balanced_data.drop('Label', axis=1), balanced_data['Label'], test_size=0.2, random_state=0)
...
# no_smote
features = new_data_pca.drop('Label', axis=1)  # Changed to Label
labels = new_data_pca['Label']  # Changed to Label
features = features.sample(n=(samples_per_class * 10), random_state=0)
labels = labels.sample(n=(samples_per_class * 10), random_state=0)
X_train_N_PCA, X_test_N_PCA, y_train_N_PCA, y_test_N_PCA = train_test_split(features, labels, test_size=0.2, random_state=0)

# no_smote
features = new_data_k_32.drop('Label', axis=1)  # Changed to Label
labels = new_data_k_32['Label']  # Changed to Label
features = features.sample(n=(samples_per_class * 10), random_state=0)
labels = labels.sample(n=(samples_per_class * 10), random_state=0)
X_train_N_32, X_test_N_32, y_train_N_32, y_test_N_32 = train_test_split(features, labels, test_size=0.2, random_state=0)
...
print(f"X_train_N_K: {X_train_N_32.shape}, y_train_N_K: {y_train_N_32.shape}, num_classes: {len(y_train_N_32.unique())}")
print(f"X_test_N_K: {X_test_N_32.shape}, y_test_N_K: {y_test_N_32.shape}, num_classes: {len(y_test_N_32.unique())}")
...
print(f"Classes in X_train_N_K: {y_train_N_32.unique()}")
print(f"Classes in X_test_N_K: {y_test_N_32.unique()}")


# save the variables
print("Saving variables...")
np.savez_compressed(os.path.join(path, 'Data_UNSW_NB15.npz'),  # Changed filename
                    ...
                    X_train_N_K=X_train_N_32, X_test_N_K=X_test_N_32, y_train_N_K=y_train_N_32, y_test_N_K=y_test_N_32)

print("Variables saved. Filesize: ", os.path.getsize(os.path.join(path, 'Data_UNSW_NB15.npz')) / 1024 ** 2, "MB")	
