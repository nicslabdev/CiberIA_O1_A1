...
def evaluate_model(model, X_train, X_test, y_train, y_test, preprocessing, is_neural=False):
    ...
    report_df.to_csv(f'/mnt/AI-DATA/alara/CiberIA_O1/Data/CIC-IDS2019_{preprocessing}_{model.__class__.__name__}_classification_report.csv')

    # Save confusion matrix as table
    cm = confusion_matrix(y_test, y_pred)
    cm_df = pd.DataFrame(cm, index=np.unique(y_test), columns=np.unique(y_test))
    cm_df.to_csv(f'/mnt/AI-DATA/alara/CiberIA_O1/Data/CIC-IDS2019_{preprocessing}_{model.__class__.__name__}_confusion_matrix.csv')
    ...
    plt.ylabel('True')
    plt.savefig(f'/mnt/AI-DATA/alara/CiberIA_O1/Data/CIC-IDS2019_{preprocessing}_{model.__class__.__name__}_confusion_matrix.png')
...
def main():
    print("Loading CIC-IDS2019 data...")
    path = os.path.join('/mnt/AI-DATA/alara/CiberIA_O1/Data')
    print("Reading data...")
    data = np.load(os.path.join(path, 'Data_CIC_IDS_2019.npz'), allow_pickle=True)
    ...
    all_results.to_csv('/mnt/AI-DATA/alara/CiberIA_O1/Data/Results_CIC_IDS_2019.csv')
