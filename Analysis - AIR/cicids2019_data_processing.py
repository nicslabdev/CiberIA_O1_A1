...
print("Reading data...")
print(f"Sampling per class: {samples_per_class} Total samples: {samples_per_class * 19}")
df = pd.read_csv(os.path.join(path, 'CIC-DDoS2019.csv'))
...
df = df[~df['Attack Type'].isin(['Portmap', 'UDPLag', 'WebDDoS'])]
...
remaining_count = (samples_per_class * 19) - stratified_df.shape[0]
...
np.savez_compressed(os.path.join(path, 'Data_CIC_IDS_2019.npz'),
...
print("Variables saved. Filesize: ", os.path.getsize(os.path.join(path, 'Data_CIC_IDS_2019.npz')) / 1024 ** 2, "MB")	
print("Data preprocessing complete.")
