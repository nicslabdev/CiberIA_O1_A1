...
class PreprocessingModule:
    ...
    def __init__(self):
        ...
        self.samples_per_class = 1950
    
    def preprocess(self, 
                  path: str,
                  raw_data: np.ndarray,
                  apply_smote: bool = False,
                  apply_pca: bool = False) -> np.ndarray:
        """
        Preprocesa los datos aplicando técnicas de limpieza y transformación.
        
        Args:
            raw_data: Datos crudos
            apply_smote: Si se debe aplicar SMOTE
            apply_pca: Si se debe aplicar PCA
        ...
        if apply_smote:
            ...
            def balance_and_split(dataset):
                ...
                for name in class_names:
                    ...
                    if len(df_class) > 1950:
                        df_class = df_class.sample(n=self.samples_per_class, replace=True, random_state=0)
                    dfs.append(df_class)
            ...
            remaining_count = (self.samples_per_class * 7) - stratified_df.shape[0]
            ...
            # Determine how many more samples to draw to reach a total of 35000 samples
            remaining_count = (self.samples_per_class * len(attacks.unique())) - stratified_df.shape[0]
        ...
        if apply_pca and apply_smote:
            ...
            print("Options: PCA and SMOTE")
        
        if apply_pca and not apply_smote:
            print("Saving variables...")
            np.savez_compressed(os.path.join(path, 'Data_CIC_IDS_2017.npz'),
            ...
            print("Variables saved. Filesize: ", os.path.getsize(os.path.join(path, 'Data_CIC_IDS_2017.npz')) / 1024 ** 2, "MB")
            print("Options: PCA and no SMOTE")

        if not apply_pca and apply_smote:
            print("Saving variables...")
            np.savez_compressed(os.path.join(path, 'Data_CIC_IDS_2017.npz'),
                                X_train_S_K=X_train_S_32, X_test_S_K=X_test_S_32, y_train_S_K=y_train_S_32, y_test_S_K=y_test_S_32)

            print("Variables saved. Filesize: ", os.path.getsize(os.path.join(path, 'Data_CIC_IDS_2017.npz')) / 1024 ** 2, "MB")
            print("Options: no PCA and SMOTE")

        if not apply_pca and not apply_smote:
            print("Saving variables...")
            np.savez_compressed(os.path.join(path, 'Data_CIC_IDS_2017.npz'),
                                X_train_N_K=X_train_N_K, X_test_N_K=X_test_N_K, y_train_N_K=y_train_N_K, y_test_N_K=y_test_N_K)

            print("Variables saved. Filesize: ", os.path.getsize(os.path.join(path, 'Data_CIC_IDS_2017.npz')) / 1024 ** 2, "MB")
            print("Options: no PCA and no SMOTE")

        return np.load(os.path.join(path, 'Data_CIC_IDS_2017.npz'), allow_pickle=True)